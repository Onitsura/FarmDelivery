...
import com.onitsura12.farmdel.utils.FirebaseHelper.Companion.COURIER_GO_TO_MESSAGE
import com.onitsura12.farmdel.utils.FirebaseHelper.Companion.COURIER_GO_TO_TITLE
...
class OrdersToDeliveryViewModel @Inject constructor() : ViewModel() {
    private val ordersList: MutableLiveData<ArrayList<Order>> = MutableLiveData()
    ...
    fun getAllDates() {
        val list: ArrayList<Order> = ordersList.value!!
    ...
    private fun sendNotificationGoTo(notification: PushNotification) = CoroutineScope(
        Dispatchers
            .IO
    ).launch {
        try {
